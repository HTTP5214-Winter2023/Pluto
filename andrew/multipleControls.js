// attempting to have control of multiple parameters each with their own controller
...
var PluginParameters = [
  ...
  function HandleMIDI(incomingEvent)
  {
    ...
    } else if((incomingEvent instanceof ControlChange) && (incomingEvent.number == 2)) {
      var newEvent2 = new TargetEvent();				// create new Target Event
      newEvent2.target = "Target 2";		// set menu entry to be used 
                            // by this event by its name
      newEvent2.value = incomingEvent.value / 127; // rescale from 0..127 to 0.0...1.0
      newEvent2.send();							// send the event
    } else
    {	
